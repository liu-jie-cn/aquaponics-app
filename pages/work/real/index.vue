<template>
	<view class="monitor">
		...
		<!-- <uni-transition :duration="500" :mode-class="['slide-left']" :show="transShow"> -->
		<view class="monitor-data">
			<view class="monitor-for" @click="monitorDetails(item,index)" v-for="(item,index) in monitorData" :key="index" v-if="item.value!=''&&item.id!=''">
...
<script>
	export default {
		...
		methods: {
				...
				uni.request({
					...
					success: (data) => {
						if (data.data.code == 200) {
							this.systeDataSettle(fieladData, data.data.data, index);
						}
					}
				})
			},
			...
			// 实时数据-系统数据梳理
			systeDataSettle(fieladData, data, index = -1) {
				// 自定义赋值数据
				let link = []
				...
				// 电机数据请求
				if (index > -1 && index < 3) this.electricalSignal(index)
				setTimeout(() => {
					this.transShow = true;
				}, 600);
			},
			...
			electricalSignal(index) {
				...
				uni.request({
					url: this.url + '/index/di/status/' + signal[index],
					header: {
						'authorization': this.token
					},
					success: (data) => {
						if (data.data.code == 200) {
							let monitor = this.monitorData.filter((fill => fill.name === '电机'))[0]
							monitor.id = signal[index];
							monitor.type = !data.data.data.offLine;
							monitor.alarmType = !data.data.data.offLine;
							monitor.value = data.data.data.deviceStatus ? '开' : '关'
						}
					}
				})
			},
